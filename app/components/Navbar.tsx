...
export default function Navbar({ players }: { players: Player[] }) {
  ...
  // ==========================================
  // 1. DEDICATED ADMIN TOPBAR / NAVBAR
  // ==========================================
  if (isAdmin) {
    const adminNavLinks = [
      { href: "/admin", label: "Dashboard", icon: "📊" },
      { href: "/admin/players", label: "จัดการนักเตะ", icon: "👥" },
      { href: "/admin/stats", label: "จัดการสถิติ", icon: "📈" },
    ];

    return (
      <nav
        className="fixed top-0 left-0 right-0 z-50 h-[72px] transition-all duration-300 border-b border-white/10"
        style={{
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full h-full">
          <div className="flex items-center justify-between h-full w-full">
            ...
            <div className="flex items-center gap-3">
              <Link href="/admin" className="flex items-center gap-2.5 group">
                <div
                  className="w-9 h-9 rounded-xl flex items-center justify-center font-display font-bold text-sm text-white shadow-lg"
                  style={{
                    background: "linear-gradient(135deg, #A2001D, #004D98)",
                    boxShadow: "0 0 15px rgba(162, 0, 29, 0.4)",
                  }}
                >
                  LM
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] px-2 py-0.5 rounded-full font-mono font-bold tracking-wider uppercase bg-[var(--barca-gold)]/10 text-[var(--barca-gold)] border border-[var(--barca-gold)]/30">
                    ADMIN
                  </span>
                </div>
              </Link>
            ...
            {/* Center: Admin Quick Tabs (Desktop) */}
            <div className="hidden md:flex items-center gap-1 bg-[var(--surface-2)] p-1 rounded-xl border border-white/5">
                ...
                return (
                  <Link
                    ...
                    className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all duration-200 ${
                      isActive
                        ? "bg-[var(--surface-4)] text-white shadow-sm border border-white/10"
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <Link
                ...
                className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[var(--barca-navy)] hover:bg-[var(--barca-navy-light)] text-white text-xs font-bold transition-all shadow-sm"
              >
                <span className="text-sm leading-none">+</span>
                <span>เพิ่มนักเตะ</span>
              </Link>

              {/* Return to Public Website */}
              <Link
                href="/"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl glass hover:bg-[var(--surface-3)] text-xs text-[var(--text-secondary)] hover:text-white border border-white/10 transition-all duration-200"
                title="กลับสู่หน้าหลักของแฟนบอล"
              >
                <span>🌐</span>
          ...
          {menuOpen && (
            <div className="md:hidden mt-3 rounded-2xl glass-dark p-3 flex flex-col gap-1 border border-white/10 shadow-2xl">
              {adminNavLinks.map((link) => {
                const isActive =
                  pathname === link.href ||
                  (link.href !== "/admin" && pathname.startsWith(link.href));
                return (
                  <Link
                    ...
                    className={`px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2.5 transition-all duration-200 ${
                      isActive
              ...
              <Link
                href="/admin/players/new"
                onClick={() => setMenuOpen(false)}
                className="px-4 py-3 rounded-xl text-sm font-bold text-white bg-[var(--barca-navy)] flex items-center justify-between mt-1"
              >
                <span>+ เพิ่มนักเตะใหม่</span>
                <span className="text-xs opacity-70">Form</span>
              </Link>
              <Link
                href="/"
                onClick={() => setMenuOpen(false)}
                className="px-4 py-3 rounded-xl text-sm font-medium text-[var(--barca-gold)] bg-[var(--surface-2)] flex items-center justify-between mt-1"
              >
                <span>🌐 กลับสู่หน้าหลักแฟนบอล</span>
